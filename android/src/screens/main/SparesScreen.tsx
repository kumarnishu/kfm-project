...
const SparesScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const { data, isSuccess, isLoading, refetch, isError } = useQuery<AxiosResponse<GetSparePartDto[]>, BackendError>(["spares"], async () => new SparePartService().GetAllSpareParts())
  ...
  const renderCard = ({ item }: { item: GetSparePartDto }) => (
    <View style={styles.card}>
      <Text style={[styles.title, { fontSize: 18 }]}>Name : {toTitleCase(item.name || "")}</Text>
