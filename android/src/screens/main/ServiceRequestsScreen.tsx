...
import { Button, ActivityIndicator, FlatList, Image, StyleSheet, View, TextInput, Text } from 'react-native';
...
import { GetServiceRequestDto } from '../../dtos/ServiceRequestDto';
...
const ServiceRequestsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const { data, isSuccess, isLoading, refetch, isError } = useQuery<AxiosResponse<GetServiceRequestDto[]>, BackendError>(["requests"], async () => new ServiceRequestService().GetAllServiceRequests())
    ...
    <View style={styles.card} onTouchEnd={() => {
      ...
      <View style={styles.cardContent}>
        <View style={styles.textContainer}>
          <Text style={[styles.title, { paddingLeft: 0 }]}>{item.customer}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput placeholder='Search' style={{ backgroundColor: 'white', marginBottom: 10 }} onChangeText={(val) => setFilter(val)} />
