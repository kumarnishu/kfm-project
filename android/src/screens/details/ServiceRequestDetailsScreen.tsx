...
import { View, StyleSheet, FlatList, Image, TouchableOpacity, Text, Button, ActivityIndicator } from 'react-native';
...
import ViewMediaDialog from '../../components/common/ViewMediaDialog';
import SelectMediaComponent from '../../components/common/SelectMediaComponent';
import { Asset } from 'react-native-image-picker';
import { UserContext } from '../../contexts/UserContext';
import { GetServiceRequestDetailedDto } from '../../dtos/ServiceRequestDto';
...
const ServiceRequestDetailsScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const { data, isSuccess, isLoading, refetch, isError } = useQuery<
    ...
    BackendError
  >(['request-detailed'], async () => new ServiceRequestService().GetServiceRequest({ id: id }));
      ...
      {user?.role == "engineer" || user?.role == 'admin' &&
        <>
          ...
          {request?.assigned_engineer && request?.solution && (
            <Button title="Close Request" onPress={() => {}} />
          )}
